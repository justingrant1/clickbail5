...
import { signUp, UserAttributes } from "@/lib/supabase"
import { createCheckoutSession, STARTER_PLAN_PRICE_ID, FREE_TRIAL_DAYS } from "@/lib/stripe"
...
export function StartTrialForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await createCheckoutSession(STARTER_PLAN_PRICE_ID)
        // Note: The checkout redirect will happen in the createCheckoutSession function
      } catch (stripeError) {
        console.error("Stripe checkout error:", stripeError)
        setErrorMessage("Failed to set up payment. Please try again.")
        setIsLoading(false)
      }
    } catch (error) {
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-6 sm:space-y-8">
            <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
                ...
                <CardContent className="p-4 sm:p-6">
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    <p className="text-white/80">
                      You'll be redirected to our secure payment processor after creating your account.
                      Your payment information is securely handled by Stripe.
                    </p>
                    
                    {errorMessage && (
                      <div className="bg-red-500/20 text-red-200 p-3 rounded-md border border-red-500/30">
                        {errorMessage}
                      </div>
                    )}
                  </div>
              ...
              <Card className="bg-slate-800/30 border-slate-700/30 backdrop-blur-sm">
                <CardContent className="p-4 sm:p-6">
                  ...
                  <p className="text-sm sm:text-base text-white/70 leading-relaxed">
                    You won't be charged for {FREE_TRIAL_DAYS} days. After your trial, you'll be automatically enrolled in the Starter
              ...
              >
                {isLoading ? "Creating Your Account..." : `Start ${FREE_TRIAL_DAYS}-Day Starter Trial`}
              </Button>
